...
- **Pages render but every panel says "Loading…" forever** — Next.js 16 dev requires the tunnel hostname in `allowedDevOrigins` (in `web/next.config.ts`). The repo already lists `*.trycloudflare.com` and `192.168.*.*` so quick-tunnel + LAN-IP demos work without edits. If you switch to a hostname outside those globs (e.g. a named tunnel on your own domain), add it to the array and restart `next dev`. Symptoms: HTML loads, navigation works, but no API requests show up in `/tmp/leaseos-api.log`.
