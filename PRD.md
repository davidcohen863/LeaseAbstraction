...
| SHA | What |
|---|---|
| (this commit) | **Code review + first pytest suite + 2 HIGH security fixes** — `CODE_REVIEW.md` catalogues 3 HIGH / 10 MEDIUM / 10 LOW findings; HIGH H1 (filename traversal on upload) and H2 (path traversal on document download) fixed via new `_safe_filename()` and `_serve_inside_sandbox()` helpers in `routes/leases.py`; HIGH H3 (in-process OAuth state) deferred; **66 pytest tests** (`tests/`) covering events math, recurring expansion, derive_events, two-pass merge, property dedup, route shape, filename sanitisation regression — all passing in ~1.4s. |
