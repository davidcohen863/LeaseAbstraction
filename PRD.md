...
**Status: ✅ shipped, with three items deliberately deferred (bbox highlight, workspace switcher, formal a11y audit).**
...
| Empty states with sample-data CTAs everywhere | ✅ (shared `<EmptyState>` rolled out everywhere) |
...
| Notification backend + bell unread count | ✅ (driven off /events; surfaces overdue + next-30d critical events) |
| Visible error states + fetch timeouts (`useApi` + `<ErrorState>`) | ✅ |
...
**Status: 🚧 demo-ready via Cloudflare Tunnel; permanent hosting still ⏸️ paused.**

| Surface | Plan | Status |
|---|---|---|
| GitHub repo | `davidcohen863/LeaseAbstraction` | ✅ pushed |
| **Demo URL (today)** | `scripts/tunnel.sh` → 2× Cloudflare quick tunnels + auto-patched env | ✅ one-command spin-up; runbook in `DEMO.md` |
| Backend hosting (permanent) | Render web + Postgres + nightly cron | ⏸️ paused — committed when Claridges says yes |
| Frontend hosting (permanent) | Vercel + Clerk auth | ⏸️ paused with backend |
| Free alternatives | Fly.io (auto-stop hobby) / HF Spaces / Cloudflare named tunnel | 💡 ready when needed |
...
| `DEPLOY.md` step-by-step | ✅ committed | Render + Vercel walkthrough |
| `DEMO.md` runbook | ✅ committed | Cloudflare Tunnel spin-up + 7-scene demo script |
...
| Item | Status | Notes |
|---|---|---|
| pytest suite for backend | ✅ | 158 tests (was 155) — added auth-coverage audit (every route either uses current_user/cron_or_user or is in INTENTIONALLY_PUBLIC) + cron_or_user prod-mode tests. ~4.2s. |
...
| SHA | What |
|---|---|
| (this commit) | **Backend hardening — Phases 6+7: auth-enforcement audit + cron_or_user dual-auth + optional Sentry — completes the "proper backend" pass.** Audit (phase 6): new `tests/test_auth_audit.py` walks every registered route at runtime and asserts each uses `Depends(current_user)` or `Depends(cron_or_user)` or is in the explicit INTENTIONALLY_PUBLIC set (health, OAuth callbacks, static helper, /docs); 48 routes covered; future endpoints added without auth fail this test immediately. Found and fixed: POST /integrations/slack/digest/run was unauthenticated (anyone could spam Slack); POST /packs/auto-trigger would 401 the cron once Clerk flips on. Both now use new `cron_or_user` dependency in api/auth.py that accepts EITHER a Clerk JWT OR an `X-Cron-Secret: $LEASEOS_CRON_SECRET` header (constant-time-compared via hmac); returns synthetic `cron` AuthenticatedUser when the secret matches so route bodies that read user.id keep working. New env var LEASEOS_CRON_SECRET in DEPLOY.md; render.yaml updated so both cron jobs pass the header. 3 new tests covering audit + dev-bypass + prod cron-secret path (no auth→401, wrong secret→401, right secret→200). Sentry (phase 7): new src/leaseos/api/observability.py with setup_sentry(env) — initialises sentry-sdk only if SENTRY_DSN env var is set (no-op otherwise so local dev needs zero config). When enabled: unhandled exceptions captured with full stack + request context, request_id attached as Sentry tag (1:1 with structured log line that produced it), user_id via set_user once auth resolves, 10% performance tracing default (SENTRY_TRACES_SAMPLE_RATE configurable), release tagged from LEASEOS_RELEASE or Render's RENDER_GIT_COMMIT. before_send_transaction filters out /healthz /readyz so orchestrator probes don't dominate the transaction list. send_default_pii=False because request bodies may contain customer lease data. attach_request_context_to_sentry called from the request-context middleware (no-op if Sentry not loaded — purely additive). setup_sentry called from create_app() after setup_logging. New dep sentry-sdk[fastapi]>=2.0.0. SENTRY_DSN, SENTRY_TRACES_SAMPLE_RATE, LEASEOS_RELEASE all documented in DEPLOY.md as optional. 158/158 backend tests passing. **All 7 backend hardening phases complete.** |
| `e104df5` | **Backend hardening — Phase 5: Stripe-style Idempotency-Key support on cost-bearing endpoints.** Prevents the classic "click submit twice" disaster where a network blip after a successful POST causes the browser to retry; without this, that meant duplicate Lease rows + duplicate Anthropic charges. New `src/leaseos/api/idempotency.py` with `idempotent(request, do_work, *, request_hash)` async wrapper: looks up inbound `Idempotency-Key` header in storage; cached + same request_hash → returns cached response with `Idempotent-Replayed: true`; cached + different request_hash → 409 Conflict; no header → pass-through. 24h TTL (matches Stripe). Cache lives at `idempotency/{key}.json` via existing storage abstraction (no new DB table or migration). Wrapped POST /leases (request_hash = sha256 of file bytes) and POST /events/{event_id}/pack (request_hash = event_id). Key sanitisation rejects non-alnum characters so the key can't escape its storage prefix. 4 new tests: no-header pass-through, same-key replay returning same lease id + replay header, same-key-different-body 409, path-traversal rejected. Test infra: new autouse `_stub_background_workers` fixture in conftest.py monkey-patches run_extraction/run_ancillary_summary/run_pack_generation to no-ops because background tasks open a fresh SessionLocal that points at the production engine not the test in-memory DB; tests that care about worker behaviour override directly. slowapi gotcha fixed: switched `Limiter(headers_enabled=False)` because slowapi tried to mutate a Response object that handlers returning Pydantic models don't expose; the 429 handler still sets Retry-After explicitly so clients can back off. Live-verified: 1st POST→201, 2nd POST same key→201 with Idempotent-Replayed:true, 3rd POST same key but different file→409. 155/155 backend tests passing. |
| `639ddd8` | **Backend hardening — Phase 4: rate limiting via slowapi.** Cost-bearing endpoints could otherwise drain the Anthropic budget under any traffic spike (accidental loop, leaked URL, runaway frontend). New `src/leaseos/api/rate_limit.py` wires slowapi with a per-user-when-authenticated / per-IP-when-not key function (prefixed `user:` / `ip:` so the same machine can't double-dip across auth states), custom 429 handler returning `{"detail": "...", "retry_after_seconds": N}` + `Retry-After` header. Limits per route: `POST /leases` 10/minute, `POST /leases/{id}/documents` 20/minute, `POST /events/{id}/pack` 10/minute, `POST /packs/auto-trigger` 3/minute (it queues N parallel Anthropic calls so it gets throttled hardest). Read endpoints unlimited. New dep slowapi>=0.1.9 with in-memory MemoryStorage (single-pod Render is fine; swap to Redis when we scale workers). Test infra: autouse `_disable_rate_limiting` fixture in conftest.py so cumulative test calls don't trip 429 mid-suite; dedicated test_rate_limit.py flips the limiter back on inside its own scope. Live-verified: 12 sequential POSTs to /leases → 10×400, then 11×429, 12×429. 151/151 backend tests passing. |
| `83ed93c` | **Backend hardening — Phase 3: real /healthz + /readyz with dependency checks.** Until now `/health` returned `{ok: true}` unconditionally — Render's healthcheck would happily route traffic to a broken instance. New `routes/health.py` splits liveness from readiness: `/healthz` (process up, sub-ms, no external calls — orchestrator restart signal) vs `/readyz` (4 checks: DB SELECT 1 round-trip + storage write/readback/delete probe + Anthropic key shape + secret-key presence — orchestrator load-balancer-pool signal, returns 503 when any dep is `down`). Each check returns a typed CheckResult with status (`ok | degraded | down | skipped`), elapsed_ms, optional detail. `degraded` (e.g. malformed Anthropic key) stays 200 because most endpoints still work; only `down` flips to 503. Anthropic check is surface-only — doesn't burn credit. Dev `secret_key` is skipped (derived per-machine key); prod missing = down. 8 new tests covering both endpoints, no-auth requirement, status-derivation logic, and a regression that the `.healthz-probe` storage key doesn't leak into `iter_prefix` listings. Inline `/health` removed from main.create_app(); aliased in the health router so old DEPLOY.md + Render config keep working. 149/149 backend tests passing. |
| `7d87d68` | **Backend hardening — Phase 2: structured logging + request IDs.** Replaces stdlib logging.basicConfig with structlog. New `src/leaseos/api/logging.py` (setup_logging configures dev pretty-printer / prod JSON-per-line based on LEASEOS_ENV; get_logger; new_request_id; contextvars for request_id/route/user_id). New `src/leaseos/api/request_context.py` RequestContextMiddleware as outermost middleware — honours inbound `X-Request-ID` header (so Cloudflare/load balancer can stamp once and we propagate), falls back to `Cf-Ray`, otherwise generates a 16-char hex; always echoes back as response header so user can quote in bug reports; logs one `request` line per request with method/path/route-template/status; uses matched route template (`/leases/{lease_id}`) not literal URL. Sweep every other logger user (worker.py, pack_worker.py, crypto.py, integrations/{slack,google,microsoft}.py) to `get_logger(__name__)` so they emit through the same pipeline and inherit per-request context. `auth.current_user` sets `user_id_ctx` after JWT verification (or dev-bypass) so every log line during the rest of the request includes who did it. New dep `structlog>=24.4.0`. 6 new tests in test_request_context.py. Live-verified: `curl -H "X-Request-ID: smoketest-abc-123" /leases` echoes the header back and logs `request method=GET path=/leases request_id=smoketest-abc-123 route=/leases status=200`. 141/141 backend tests passing. |
| `c2bd48a` | **Backend hardening — Phase 1: storage abstraction.** First chunk of the "proper backend" work. New `src/leaseos/api/storage.py` with a `Storage` Protocol + `LocalStorage` implementation; logical key namespace (`documents/`, `packs/`, `templates/`); `get_path()` context-manager escape hatch for libs that need a real path; `coerce_to_key()` back-compat shim so existing absolute-path DB rows resolve via the new layer without a migration. All five file-IO call sites refactored: lease upload, side-letter attach, lease + pack delete (cleans bytes + tree dirs), pack worker (render to temp + upload via storage.put), extraction worker (`get_path` ctx for PyMuPDF), template upload/list/delete/download, `pack_generator._firm_template_path` so firm letterhead survives even when storage is remote. New `LEASEOS_STORAGE_BACKEND` env var (`local` default, `s3` raises NotImplementedError until cloud deploy). Test infra: new autouse `_reset_storage_cache_per_test` fixture; three template tests rewritten to write through `get_storage().put` (the prod path) rather than dropping files directly. 135/135 backend tests passing. Live verification on dev DB confirms legacy absolute-path rows still resolve cleanly. Next phases land as separate commits: structured logging + request IDs, real /healthz + /readyz, rate limiting, idempotency keys, auth-enforcement audit, optional Sentry. |
| `21d6c81` | **P2.2 CRUD + UX polish — destructive operations, toast system, confirm-dialog, row-action menus, inline rename.** Motivated by user pushback that the previous "pilot-grade" claim was wrong: you couldn't delete a lease, every action was either silent or used native `alert()`/`confirm()`, and there was no inline rename anywhere. **Backend:** new `DELETE /leases/{id}` (cascades documents/events/packs/edits via SQLAlchemy + cleans on-disk PDFs and `data/packs/<pack_id>/` dirs after the DB commit; preserves Property and Comparable.derived_from_lease_id is null'd rather than cascading), `DELETE /packs/{id}` (cascades pack_documents + cleans `data/packs/<id>/`; refuses settled packs with a 400 because the settled rent has fed comparables + audit), `DELETE /properties/{id}` (refuses if leases attached unless `?force=true`, in which case leases are unlinked and survive), `PATCH /leases/{id}` (rename label, audit-logged via FieldEdit with synthetic field_path `__label__`). 14 new tests covering cascade math + invariants. **Frontend foundations:** `<ToastProvider>` + `useToast()` (bottom-right toast cards, 4s auto-dismiss for success/info, 7s for errors), `<ConfirmHost>` + `useConfirm()` imperative API backed by a native `<dialog>` modal with focus-trapping/ESC/backdrop-click for free, `<RowActions>` three-dot ⋯ menu used on every list table. **Rolled out everywhere:** /leases table (Rename + Delete), /leases/[id] header (inline-editable label with pencil icon, Enter/Esc keyboard handling + RowActions menu), /packs table + /packs/[id] header (Delete with disabled-when-settled state), /properties table + /properties/[id] header (Delete with smart force-when-leases-attached confirm copy). Zero `alert()` and zero `window.confirm()` left in the app — every action that previously fell through to a browser dialog now uses the proper components. The 7 toast-and-confirm migration touched 6 component files (templates, comparables, calendar, calendar-drawer, reviews, lease-detail RightRail). 135/135 backend tests passing (was 121); frontend `tsc --noEmit` clean. Honest gaps still open: no bulk delete on the leases selection toolbar yet (checkbox column only enables CSV export today); /leases table rename uses `window.prompt` because a proper inline-row-edit would be a bigger refactor; no undo for destructive actions (would need soft-delete); no frontend test runner. |
| `a479601` | **P2.1 UI completion** — finishes the P2 milestone in one sweep. (1) `useApi` + `<ErrorState>` rolled to every fetching page (8 list pages + 3 detail pages); every fetch has a 15s timeout and a real "Couldn't load — retry?" card on failure. (2) Shared `<EmptyState>` standardises the bespoke empty cards on `/leases`, `/properties`, `/comparables` — every empty state now tells the user what to do next. (3) Notification bell is real — drives off `GET /events?days_ahead=60&days_behind=30`, surfaces overdue + critical-next-30d items (rent_review_trigger / break_notice_deadline / lease_expiry / epc_expiry); badge red if anything overdue, amber otherwise; auto-refreshes every 5 min. (4) Per-firm Word template upload at `/settings/templates` (was a placeholder) — new `routes/templates.py` with GET / POST / DELETE / download per kind, files at `data/templates/{kind}.docx` (single-tenant convention for v1), `pack_generator.render_docx()` opens the firm template as the base document if present so letterhead + logo + footer styles + boilerplate survive into generated packs. Validates .docx server-side via python-docx parse before persisting (rejects garbage with right extension), 10 MB max, sanitises original filename. `serve_inside_sandbox` extended to allowlist `data/templates/`. 9 new tests pin the round-trip + render-uses-template behaviour; 121/121 backend tests passing (was 112). Frontend `tsc --noEmit` clean. Deferrals: bbox highlight on citation click (needs backend bbox extraction), workspace switcher (post-pilot per CLAUDE.md), formal a11y audit (iterative not one-shot), frontend test runner (vitest tracked but not added). |
| `a9bf545` | **Frontend hardening: 15s fetch timeouts + `useApi` hook + `<ErrorState>` so silent loading-spinners are now visible "couldn't load — retry?" cards.** Motivated by the recent 20-minute "everything is loading" debug where Next 16's `allowedDevOrigins` blocked client modules and `useEffect` never fired — there was no error trail. `lib/api.ts` `call()` now wraps every fetch in an `AbortController` with a 15s default timeout, composed with the caller's signal via `AbortSignal.any` so unmount cancels still work; introduces typed `ApiTimeoutError(path, timeoutMs)` and `ApiHttpError(path, status, body)` so the UI can differentiate timeout / 401 / 404 / 5xx. New `lib/use-api.ts` `useApi<T>(loader, deps)` hook returns `{ data, loading, refetching, error, refetch }` — replaces the `useState(null) + useEffect → setState` pattern that was repeated on every page; auto-cancels on unmount or dep change; keeps prior `data` populated during refetches so panels don't flash blank. New `components/ui/error-state.tsx` `<ErrorState>` card pairs with `<EmptyState>`, recognises the typed errors to show useful copy ("took longer than 15s — check your connection"), Retry button calls back into `useApi.refetch`. Rolled out to `/today` (3 queries combined into one banner + retry-all), `/leases` (3 queries; upload errors retain their own banner), `/properties` (1 query), `/packs` (1 query). Already proved itself live: the tunnel briefly went down mid-session and the user saw the new ErrorState's "Couldn't reach the API" message instead of an eternal spinner. Frontend `tsc --noEmit` clean; backend 112/112 still passing. No frontend test runner in place yet — tracked as a follow-up since adding vitest just for these would be heavier than the change itself. |
| `13b1406` | **Hotfix: Next 16 `allowedDevOrigins` for tunnelled demo URLs.** The Cloudflare tunnel loaded the app shell but every data panel stayed in its loading state forever — `useEffect` never fired, no API requests left the browser. Cause: Next.js 16 dev refuses to fully serve internal Turbopack assets (chunks, RSC payload, HMR socket) when the browser's `Origin` header doesn't match the dev server's bound host. Fix: `web/next.config.ts` now sets `allowedDevOrigins: ["*.trycloudflare.com", "192.168.*.*", "127.0.0.1", "localhost"]` so quick-tunnel demos and LAN-IP access (phone on same Wi-Fi) work out-of-the-box. Production builds ignore the setting. DEMO.md troubleshooting section gets a new entry pointing at this exact failure mode + fix. |
| `527a08a` | **Hotfix: `settle_pack` was writing the entire `permitted_use` clause text into `Comparable.use_class`** instead of the short Use Classes Order code (`E(b)` etc.) — broke the comparables filter UI and showed 121-char legal paragraphs in the rent-review pack drawer. Added a regex over the clause text matching `sui generis | E(a)-E(g) | E | F1 | F2 | B[12] | C[1-4]`, using `(?!\w)` lookahead per alternative because a naive `\b...\b` fell through to bare `E` on `E(b)` inputs (closing `)` and following space are both non-word so `\b` failed there). 14 new tests pin the behaviour against real extraction samples + edge cases. Cleaned up the three pre-existing bad comparable rows in the local dev DB in-place. 112/112 backend tests passing (was 98). |
| `614cc76` | **Cloudflare Tunnel runbook (`scripts/tunnel.sh` + `DEMO.md`).** `scripts/tunnel.sh` (~30s, one command) starts two `cloudflared tunnel --url` quick tunnels (no Cloudflare account needed) — one for the API on :8000, one for the Next dev server on :3002 — parses the `*.trycloudflare.com` URLs out of cloudflared's stderr, patches `LEASEOS_CORS_ORIGINS` in `.env` to include the frontend hostname, overwrites `NEXT_PUBLIC_API_URL` in `web/.env.local` with the API hostname, restarts uvicorn + `next dev` so the env changes take effect, smoke-tests `/health` + `/today` through the tunnels, and prints the public URL to share. `scripts/tunnel.sh stop` tears it all down. `DEMO.md` is the Claridges-demo playbook: pre-flight (reseed demo lease + comparables + pre-generate one pack so the click-to-pack scene is instant), the 12-minute seven-scene walk-through (Today → Reviewer with j/k → Calendar → Reviews → Pack → Settings → Q&A), the FAQ answers, what's deliberately not in the demo (so we don't promise bbox highlighting or templates), and tear-down. Tunnels are ephemeral; multi-day pilot would need a named tunnel via Cloudflare Dashboard. CORS preflight verified end-to-end (200 + correct ACA-Origin echo). |
| `0d94ace` | **PDF worker version-lock** — hotfix for the lease-detail viewer crash `UnknownErrorException: The API version "5.4.296" does not match the Worker version "5.7.284"`. The self-hosted worker at `web/public/pdf.worker.min.mjs` had been copied from the project's top-level `pdfjs-dist@^5.7.284` dep, but `react-pdf@10.4.1`'s `<Document>` resolves its own nested `pdfjs-dist@5.4.296` and pdf.js refuses to run when worker and API don't match. New `sync-pdf-worker` script in `web/package.json` resolves the worker via `require.resolve('pdfjs-dist/build/pdf.worker.min.mjs', {paths:[require.resolve('react-pdf')]})` and copies it into `public/`; wired as `postinstall` so `npm install` keeps it in lockstep on every machine and on Vercel. Re-copied the correct 5.4.296 worker as part of this commit. |
